...
export default function Video() {
    ...
    const result = async() => {
        const assistant = await openai.beta.assistants.create({
            name: "Immersed Narrator",
            instructions:  `You are an expert storyteller telling the story of Harry Potter and the Sorcerer's Stone in a choose-your-adventure type game. You are trained to write in the style of the author J.K. Rowling. Here's some information about J.K. Rowling's writing style:
            Accessible and Descriptive:
                * Uses straightforward language and clear descriptions to paint a vivid picture of characters and settings.
                * Balances simplicity with attention to detail, making it accessible to a wide audience.
            Character-Centric:
                * Introduces characters with distinct traits, allowing readers to quickly form connections with them.
                * Blends physical descriptions with character behaviors, providing a well-rounded understanding.
            Blend of Mundane and Magical:
                * Seamlessly integrates the ordinary with the extraordinary, creating a world where magical elements coexist with everyday life.
                * Describes magical occurrences with a matter-of-fact tone, adding a touch of whimsy.
            Emphasis on Observation:
                * Utilizes characters' observations to convey information about the world and advance the plot.
                * Details characters noticing peculiar events, inviting readers to join in their sense of curiosity.
            Subtle Humor:
                * Incorporates humor into character interactions and observations, lightening the tone.
                * Balances serious and whimsical elements, appealing to a broad audience.
            Hints and Mysteries:
                * Introduces elements without immediate explanations, creating an air of mystery and intrigue.
                * Encourages readers to wonder about the significance of certain details.
            Scene Transitions:
                * Smoothly transitions between scenes, maintaining a cohesive narrative flow.
                * Alternates between character perspectives and locations to provide a comprehensive view.
            Realistic Dialogue:
                * Develops characters through natural and authentic dialogue, capturing their personalities.
                * Uses dialogue to convey important plot points while maintaining a conversational tone.
            You should mimic this writing style and generate a story continuation of at least 3 paragraphs in length AND a list of options for continuing the story.`,
            tools: [],
        ...
        const message = await openai.beta.threads.messages.create(thread.id, {
            ...
            role: "user",
            content: `Generate a story continuation of 1-3 paragraphs in length AND a list of options to allow the user to choose how to continue the story. You MUST respond in the following JSON format:
            {
            "story": "...",
            "options": [
                "...",
            ...
            Make sure to follow the general outline below but be creative with the outcomes of the story and choices given. We want the user to take full control of Harry Potter's life throughout the story and make key decisions along the way. Make sure to take each section slowly, and really flesh out the scene, characters, and actions.
            Here is a general outline of the elements of Harry Potter the Sorcerer's Stone that must happen at some point:
            Chapter 1:
            ...
            * Irony: Harry's modest reaction to his fame contrasts with Draco's entitlement, emphasizing Harry's humility and grounded nature.
            ...
            Here are themes, setting, and character information that will be key in the generation of the text. Make sure to use this to describe different aspects of the story in a creative way:
            Theme 1:
            ...
            * 		The Third-Floor Corridor: A forbidden area in Hogwarts, guarded by a series of magical obstacles. It houses the Sorcerer's/Philosopher's Stone and is a crucial location for the climax of the story. The corridor represents the dangers and mysteries hidden within the castle.
            Here are some key characters to include throughout the story, adhere to visual descriptions and personalities:
            Harry Potter
    ...
    const checkAndReceiveRun = async () => {
        let run = await openai.beta.threads.runs.retrieve(
          ...
          if (lastMessageForRun) {
            let story = lastMessageForRun.content[0].text.value;
            if (story.includes("```json") || story.includes("```")) {
                story.replace("```json", "");
                story.replace("```", "");
            }
            console.log(story);
    ...
    const createRun = async() => {
        const run = await openai.beta.threads.runs.create(threadId, {
            ...
            instructions: "Continue with the story."
        })
        setRunId(run.id);
    }
        ...
        <div className="grid place-items-center h-screen">
            <div className="w-3/4">
                ...
                <div className="mt-3 rounded-md p-3 overflow-y-auto">
                    <p className="font-mono">In the corner of Privet Drive, nestled between perfectly pruned rose bushes and immaculately trimmed hedges, stood number four – home to the exceedingly ordinary Dursley family and the distinctly unordinary Harry Potter. Ten long years had passed in this suburban purgatory, and each had etched deeper the lines of misery on young Harry’s forehead. A mop of unruly black hair crowned his thin face, doing little to conceal the lightning-shaped scar that lay beneath like a secret kept from the world. As Dudley Dursley, the inflated balloon of a boy, reveled in his eleventh birthday, his voice swelled with indignation upon counting thirty-six presents – one fewer than the previous year. The ensuing tempest of tantrums would have outdone even the fiercest squall at sea. "Thirty-six?! But last year—last year I had thirty-seven!" Dudley's face morphed into a shade of puce that clashed horrendously with his new racing bike. Harry Potter, observing from a cautious distance, could not help but feel the tiniest flicker of satisfaction each time Dudley’s piggy eyes roamed over his gifts and found them inadequate. But Harry’s quiet amusement was always tinged with wariness, for Dudley’s happiness was a precarious thing, bought with any toy his heart desired, and Harry knew all too well the price of Dudley's displeasure. The celebratory day found its climax in a visit to the zoo, an event which Dudley insisted was necessary to amend the deficit in presents, and which Vernon Dursley agreed to with a resigned nod, fearing public tantrums more than public expenditure. Harry, dressed in Dudley's old clothes – far too large and smelling faintly of disdain – was brought along like an afterthought. Upon entering the reptile house, Harry, accustomed to being ignored, drifted away from the Dursleys. There, in the shadowy tank, he found the boa constrictor, its dark, wise eyes holding a kind of sadness that Harry knew all too well. He stood there, transfixed, feeling an unspoken understanding pass between them. It was then that Harry heard a peculiar voice, one no one else seemed to notice. "Brazil, I reckon," said the snake, nearly causing Harry to jump out of his oversized shoes. Who spoke? Did the snake just...?</p>https://personal-aiatl2.openai.azure.com
                </div>
                <br />
                <p className="font-mono font-semibold">What will you do next?</p>
                <div className="mt-5 grid gap-x-8 grid-cols-3">
                    <motion.a whileHover={{ scale: 1.1, backgroundColor: 'lightgray' }} whileTap={{ scale: 0.8 }} class="bg-slate-200 rounded-md p-3">
                        <p className="font-mono font-semibold text-slate-800">Attempt to communicate with the snake further, incurring the possibility of discovery but also forging an unprecedented connection.</p>
                    </motion.a>
                    <motion.a whileHover={{ scale: 1.1, backgroundColor: 'lightgray' }} whileTap={{ scale: 0.8 }} class="bg-slate-200 rounded-md p-3">
                        <p className="font-mono font-semibold text-slate-800">Attempt to communicate with the snake further, incurring the possibility of discovery but also forging an unprecedented connection.</p>
                    </motion.a>
